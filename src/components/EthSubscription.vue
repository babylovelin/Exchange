<template lang="html">
  <div class="rengoupage">
    ...
    <div class="dhbl">
      <p>1&nbsp;&nbsp;TCH&nbsp;=&nbsp;0.0008&nbsp;ETH</p>
    </div>
    <div class="coincanbuy">
      <span class="canuse">可用&nbsp;&nbsp;{{eth.toFixed(8)}}&nbsp;ETH</span>
    ...
    <div class="marketcoinneed">
      总额 &nbsp;&nbsp;{{(buynum*0.0008).toFixed(8)}}&nbsp;&nbsp;ETH
    </div>
    <div class="statement">
      如果您已达到认购上限，想继续认购Tcash，可以通过我们的合作伙伴渠道进行认购
    </div>
    <div class="duihuan">
      <span @click="byeth">以ETH兑换</span>
    </div>
  </div>
</template>
...
methods:{
  jumptocz:function(){
    this.$router.push('/main/userInfo/recharge/'+'eth')
  },
  byeth:function(){
    if(this.tch>40000){
      alert("您已经超出单人认购数量！")
    ...
    var r=confirm("您确定要认购吗？以"+this.buynum*0.008+'ETH认购'+this.buynum+'Tcash')
    if(r){
      var params = new URLSearchParams();
      params.append('uid', localStorage.uid)
      params.append('market_num',this.buynum*0.08)
      params.append('market_kind', 'eth')
